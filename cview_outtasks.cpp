...
void CView_OutTasks::OnUpdate(CView *pSender,LPARAM lHint,CObject *pHint)
{
 ...
 else 
 {
  ...
  CUser cUser_Out;
  if (cDocument_Main_Ptr->GetCVectorUser().FindByUserGUID(cUser.GetUserGUID(),cUser_Out)==false)//такого пользовател€ нет в базе, значит, был выбран проект
  {
...
afx_msg void CView_OutTasks::OnCommand_Menu_List_TaskEdit(void)
{
 ...
 while(1)
 {
  ...
  {   
   if (cTask.IsForUserGUID(for_user_guid)==false)//у задани€ помен€лс€ адресат
   {
