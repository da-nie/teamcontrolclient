...
bool CTaskExportCSV::Export(const CString &file_name,const std::list<CTask> &list_CTask,const std::list<CUser> &list_CUser,const std::list<CProject> &list_CProject)
{
 ...
 while(iterator!=iterator_end)
 {
  ...
  fprintf(file,";");
  if (cTask.IsStateNoRead()) fprintf(file,"Не прочитано");
  if (cTask.IsStateReaded()) fprintf(file,"Прочитано");
  if (cTask.IsStateIsRunning()) fprintf(file,"Выполняется");
  if (cTask.IsStateDone()) fprintf(file,"Выполнено");
  if (cTask.IsStateCancelled()) fprintf(file,"Отклонено");
  if (cTask.IsStateFinished()) fprintf(file,"Завершено");
  fprintf(file,";");
