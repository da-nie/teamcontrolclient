...
bool CTaskExportHTML::Export(const CString &file_name,const std::list<CTask> &list_CTask,const std::list<CUser> &list_CUser,const std::list<CProject> &list_CProject)
{
 ...
 while(iterator!=iterator_end)
 {
  ...
  fprintf(file,"</td>\r\n");
  if (cTask.IsStateNoRead()) fprintf(file,"<td width=\"10%%\">Не прочитано</td>\r\n");
  if (cTask.IsStateReaded()) fprintf(file,"<td width=\"10%%\">Прочитано</td>\r\n");
  if (cTask.IsStateIsRunning()) fprintf(file,"<td width=\"10%%\">Выполняется</td>\r\n");
  if (cTask.IsStateDone()) fprintf(file,"<td width=\"10%%\">Выполнено</td>\r\n");
  if (cTask.IsStateCancelled()) fprintf(file,"<td width=\"10%%\">Отклонено</td>\r\n");
  if (cTask.IsStateFinished()) fprintf(file,"<td width=\"10%%\">Завершено</td>\r\n");
  fprintf(file,"</tr>\r\n");
  iterator++;
  index++;
 }
